...
- 内置 DeepSeek Flash 支持让 Echo 在获得文件授权后查看本地图片。
- 自定义连接仅在使用 OpenAI 官方服务地址（`https://api.openai.com/v1`）、选择 Responses 协议并启用“图片输入”时开放此能力；所选模型本身也必须支持图片。该设置不会替你验证所有模型的实际能力。
- 当前 Kimi、纯文本模型、Chat Completions 连接及其它兼容服务地址不开放本地看图工具。能添加图片附件并不代表同一连接也能接收工具读取的图片。
- 不支持时不会自动调用其它视觉模型。具体操作与图片大小限制见[本地图片读取](using-agent-friend.md#让-echo-查看本地图片)。
